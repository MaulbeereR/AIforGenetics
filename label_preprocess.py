...
metadata = pd.read_csv('~/Desktop/AIforGenetics/filtered_FCS_data_2024-03-07_V2.csv')
print(metadata.head())    

def process_raw_data(cell_count, input_path, metadata, sample_count):
    data_path = os.path.join(input_path, 'data')

    input_filelist = os.listdir(data_path)
    ...
    for f_idx, fcs_filename in enumerate(input_filelist):
        ...
        if f_idx <= sample_count:
            ...
            cell_data = common.read_FCS(os.path.join(data_path,fcs_filename))
            print(cell_data.shape)

            if cell_data.shape[0] > cell_count:
                random_indices = np.random.choice(cell_data.shape[0], size=cell_count, replace=False)
                trimmed_data = cell_data[random_indices, :]
    ...
    all_fidx = np.stack(all_fidx_list)

    return all_data, all_labels, all_fidx
...
print(os.path.exists(label_filename))
print(os.path.exists(idx_filename))
